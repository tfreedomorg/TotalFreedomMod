...
public class LockupManager extends FreedomService
{
    ...
    public LockupManager(TotalFreedomMod plugin)
    {
        super(plugin);
    ...
    {
        return !plugin.al.isAdmin(player) && heldOn(plugin.pl.getPlayer(player).getIp());
    }
    ...
    {
        return !plugin.al.isAdminSync(player) && heldOn(plugin.pl.getPlayerSync(player).getIp());
    }
    ...
    {
        final String ip = plugin.pl.getPlayer(target).getIp();
    ...
    public Optional<LockupEntry> findByTarget(String target)
    {
        ...
        if (online != null)
        {
            final LockupEntry byAddress = lockups.get(plugin.pl.getPlayer(online).getIp());
            if (byAddress != null)
    ...
    private List<Player> playersOn(String ip)
    {
        return server.getOnlinePlayers()
                     .stream()
                     .map(Player.class::cast)
                     .filter(player -> ip.equals(plugin.pl.getPlayer(player).getIp()))
                     .toList();
    }
    ...
    private void load()
    {
        ...
        {
            FLog.severe(String.format("Could not read lockups.json: %s", ex.getMessage()));
        }
    ...
    private synchronized void writeJson(Map<String, LockupEntry> snapshot)
    {
        ...
        {
            FLog.severe(String.format("Could not save lockups.json: %s", ex.getMessage()));
        }
    }
}