...
public class LibsDisguisesBridge extends FreedomService
{
    ...
    private boolean registerDisguiseListener()
    {
        ...
        {
            FLog.warn("LibsDisguises is loaded but DisguiseEvent was not found; apply-time disguise "
                + "checks are INACTIVE and only command filtering applies.");
        }
        catch (Exception ex)
        {
            FLog.error("Failed to register apply-time disguise checks: " + ex.getMessage());
            FLog.error(ex);
        }
        return false;
    }
